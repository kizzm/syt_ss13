...
Für die Integration in die Simulation müssen vorab einige Berechnungen angestellt werden.
Als erstes wird die benötigte Beschleunigung berechnet, die mit einem Drehmoment auf den Spiegel übertragen werden muss, um den Spiegel mit seinem Trägheitsmoment über 
seinen maximalen Ablenkwinkel zu bewegen.

Dabei werden folgende Vorgaben zu Grunde gelegt:
\begin{itemize}
\item Maximale Ablenkung von \unit[20}{°}
\item Maximale Zeit für Ablenkung \unit[1]{ms}
...
Es ergibt sich eine Druchschnittswinkelgeschwindigkeit von unit[349]{rad/s}, um einen Winkel von \unit[20}{°} in \unit[1]{ms} zu überfahren.
...
wird ein linearer Verlauf der Geschwindigkeit von $\omega = \unit[0]{rad/s}$ und der doppelten Durchschnittsgeschwindigkeit $\omega = \unit[698]{rad/s}$ bei der Hälfte der 
...
\end{equation}
\end{center}
Der Spiegel erfährt zu Beginn der Regelung eine Beschleunigung von $\alpha = \unit[1,396 *10^6]{rad/s^2}$ um nach der Hälfte der Zeit, also nach 0,5 ms wieder mit dem gleichen
...
\label{equ:M_last}
M_L = J * \alpha\\
M_L = \unit[93,3 * 10^{-9}]{kg m^2} * \unit[1,396 *10^6]{rad/s^2} \\
...
für den Motor $U_e$, siehe Abb. \ref{fig:motorelek}.
...
Moment-Winkel-Beziehung (Reibung) $M_R = r*\omega$, siehe Abb. \ref{}.
...
\end{equation}
\end{center}
Für die spätere Integration in Simulink wird die Gleichung \ref{equ:motormomentebilanz} umgeschrieben:
\begin{center}
\begin{equation}
