...
Für die folgenden Simulationen wird das Matlab-File \texttt{msSpiegel{\_}PID.m} und das Simulink-File \texttt{sSpiegel.slx} verwendet.
\begin{figure}[!h]
	\centering
...
Eine Eröhung des P- oder I-Anteils ergibt ein Überschwingen, wie es in Abb. \ref{fig:p30i18} dargestellt ist.
...
Für die folgenden Simulationen wird das Matlab-File \texttt{msSpiegel{\_}Pad.m} und das Simulink-File \texttt{sSpiegelPad.slx} eingesetzt.
\begin{figure}[!h]
	\centering
...
Um heraus zu finden, welche Daten der Motor aufweisen muss, um mit einer PID- oder P-Adaption geregelt werden zu könnnen, werden jetzt zusätzlich zu den $f_{1}$ und $f_{2}$
Parametern auch die Motorparameter geändert.
Es wurden Werte für den Innenwiderstand und der Induktivität eines Galvos 6230 der Firma Cambridge Technology als Grundlage verwendet \cite{CaTe}.

CaTe: PDF, Model 6230H Optical Scanner (Mechanical and Electrical Specifications), Cambridge Technology, 03/07.

Für die folgenden Simulationen wird das Matlab-File \texttt{msSpiegel{\_}Pad{\_}Werte.m} und das Simulink-File \texttt{sSpiegelPad.slx} hergenommen.
...
\item Induktivität der Spule: $\unit[173]{\mu H}$
...
Sollte die Regelung erfolgreich sein, kann mit den veränderten Werten evtl. ein Motor und Spiegel hergestellt werden, der den Anforderungen entspricht.

Für die folgenden Simulationen wird das Matlab-File \texttt{msSpiegel{\_}Pad{\_}Neue{\_}Werte.m} und das Simulink-File \texttt{sSpiegelPad.slx} verwendet.
\begin{figure}[!h]
	\centering
...
\item Induktivität der Spule: $\unit[3]{\mu H}$
\item Motorkonstante KMPHI: $\unit[35e-3]{V s}$
\item Reibungskoeffizient: $\unit[6e-5]{N m s}$
\item Trägheitsmoment des Spiegels: $\unit[93.3e-9]{kg m^2}$
...
Es fliessen Ströme in Höhe von $\unit[80]{A.}$
\begin{figure}[!h]
	\centering
...
Für eine bessere Übersicht, wurden entsprechende Positionen mit einem Namen versehen und der D-Anteil aus der Regelung genommen, da dieser auf Null gesetzt ist, siehe Abb. \ref{fig:strombegrenztsauber}.
\begin{figure}[!h]
	\centering
...
\end{figure}
Für die folgenden Simulationen wird das Matlab-File \texttt{msSpiegel{\_}Pad{\_}Neue{\_}Werte{\_}Strom.m} und das Simulink-File \texttt{sSpiegelPadStrom.slx} hergenommen.
...
\begin{itemize}
\item Neues Trägheitsmoment des Spiegels: $\unit[93.3e-11]{kg*m^2}$
...
\begin{itemize}
\item Trägheitsmoment des Spiegels: $\unit[93.3e-11]{kg*m^2}$
...
Dafür wird das vorhandene Simulink-File \texttt{sSpiegelPadStrom.slx} hergenommen und um den Sensor erweitert.
...
Auch hier ist eine restliche Regeldifferenz zu erkennen, die sich wiederum nicht durch anpassen der Regelparameter begleichen lässt.
